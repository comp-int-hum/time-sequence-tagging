...
def filter_encoded_sentences(sentences, patterns):
	return [s for s in sentences if not any(re.match(pattern, s["text"]) for pattern in patterns)]
    ...
    with gzip.open(args.input, "rt") as input_file, gzip.open(args.output, "wt") as output_file:
        for i, line in tqdm(enumerate(input_file), desc = "Looping over works"):
            ...
            for chapter_dict in chapters:
                ...
                for par in chapter:
                    filtered_sentences = filter_encoded_sentences(par, compiled_filters)
