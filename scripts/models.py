...
class MulticlassSequenceTaggerWithBahdanauAttention(nn.Module):
    ...
    def __init__(self, input_size, label_classes, hidden_dim = 512):
        # input: (N, L, H_in), output: (N, L, D * H_out) where D = 2 if bidirectional, 1 otherwise
        ...
        for labels in label_classes:
            print(f"Label length: {len(labels)}")
            self.output.append(nn.Linear(in_features = hidden_dim * 2, out_features = len(labels)))
    ...
    def forward(self, sentence_embeds, labels = None, device = "cpu"):
        ...
        # forward output shape: [batch, hidden_dim]
        combined_outputs = [torch.cat((f, b), dim = 1) for f, b in zip(forward_outputs, backward_outputs)]
        preds = [torch.stack([output_layer(result) for result in combined_outputs], dim = 1) for output_layer in self.output] # [batch_size, seq_len, output_classes]
